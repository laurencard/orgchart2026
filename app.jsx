    ...
    function isManagementPerson(name) {
      if (!name || name === "TBD") return false;
      const n = name.trim();
      return MANAGEMENT_PATTERNS.some(re => re.test(n));
    }
    ...
    function mgmtPerson(displayName, title, roster) {
      const found = findInRoster(roster, displayName.split(" ")[0].toLowerCase(), displayName.toLowerCase());
      return makePerson(displayName, title, found?.workingHours ?? null, false, found?.photoUrl ?? null);
    }
    ...
    function buildInitialData() {
      ...
      const leeBranch = makeTeam("Chief of Staff", mgmtPerson("Lee", "Chief of Staff", roster), [], [
        makeTeam("Sales", mgmtPerson("Leo Loomie", "Sales", roster)),
        makeTeam("Success", mgmtPerson("Matt", "Success", roster)),
        makeTeam("Ops", mgmtPerson("Jana", "Ops", roster)),
        makeTeam("Pods — Christina", mgmtPerson("Christina", "Pods", roster)),
        makeTeam("Pods — Hayle", mgmtPerson("Hayle Kluesner", "Pods", roster)),
      ], null, "blue");

      const jonathanBranch = makeTeam("COO", mgmtPerson("Jonathan Spinetto", "COO", roster), [], [
        makeTeam("Platform", mgmtPerson("Shubi", "Platform", roster)),
        makeTeam("Finance", mgmtPerson("Nicole", "Finance", roster)),
        makeTeam("Risk", mgmtPerson("Obi", "Risk", roster)),
      ], null, "violet");

      const management = makeTeam("NFTY", mgmtPerson("Mark Schacknies", "CEO", roster), [], [leeBranch, jonathanBranch]);

      const seen = new Set();
      const unallocatedPeople = roster
        .filter(p => !isManagementPerson(p.name))
        .filter(p => {
        ...
        .map(p => makePerson(p.name, p.title || p.role || "", p.workingHours ?? null, p.isTBD, p.photoUrl ?? null));
